...
public class TwistedVineFeature extends Feature<DefaultFeatureConfig> {

	public TwistedVineFeature(Codec<DefaultFeatureConfig> configCodec) {
		super(configCodec);
	}
	...
	public boolean generate(StructureWorldAccess world, ChunkGenerator chunkGenerator, Random random, BlockPos pos, DefaultFeatureConfig config) {
		...
		if (random.nextBoolean() && random.nextBoolean() && random.nextBoolean()) {
			...
			double x = MathHelper.lerp(random.nextDouble(), -45, 45);
			double y = random.nextDouble() * 360;
			double z = MathHelper.lerp(random.nextDouble(), -45, 45);

			double width = random.nextDouble() * 2 + 1;
			double length = random.nextDouble() * 2 + 1;

			double helixHeight = height / (random.nextDouble() * 3 + 2);
			double radius = random.nextDouble() * width + (length / 2);
