		...
		if (random.nextBoolean() && random.nextBoolean() && random.nextBoolean()) {
			...
			shape.applyLayer(new AddLayer(
					...
					.validate(new SafelistValidator(world, VALIDATE_WHITELIST), (validShape) -> {
						validShape.fill(new WhitelistedRandomSimpleFiller(world, GardenBlocks.DEAD_TREE.leaves.getDefaultState().with(LeavesBlock.PERSISTENT, true), world.getRandom(), 0.3F, WHITELIST));
					});
